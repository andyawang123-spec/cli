...
func readTransferEvent(runtime *common.RuntimeContext, calendarID, eventID string) (*calendarEvent, error) {
	data, err := runtime.CallAPITyped("GET",
		...
		nil, nil)
	if err != nil {
		return nil, withStepContext(err, "failed to read event %s while checking whether it is recurring; pass --%s to confirm a whole-series transfer and skip this check", eventID, flagTransferSeries)
	}
