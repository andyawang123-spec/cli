...
模板中心是公开的 Base 模板库，不是用户云空间里的已有 Base。用户想用现成模板创建新 Base，且没有指向已有对象的锚点（没有 Base URL、没有“我的/最近访问的表”、没有具体已存在的 Base 名）时，可读取 [模板中心](references/lark-base-template-center.md) 查找模板中心模板，并使用：

- `+template-categories`：列出模板分类，拿到 `categories[].key` 后作为 `+template-list --category-key`。
- `+template-list`：列出某个分类下的模板；不传分类时返回推荐类目。
- `+template-search`：按业务关键词搜索公开模板；关键词不能为空。

三个命令都是只读，默认使用 `--as user`，需要 `base:template:read`。分类浏览时 `+template-list --category-key` 必须使用 `+template-categories` 返回的 `categories[].key`，不要凭空猜类目 key；分页、字段含义和完整流程见 reference。

模板中心返回的 `templates[].token` 是模板 Base token，`templates[].link` 只用于预览和确认模板，不代替 token。选定模板后用 `+base-copy --base-token <token>`（这里的 `<token>` 就是模板 Base token）复制成用户自己的新 Base；用户未指定新名称时可直接使用模板的 `name` 作为 `+base-copy --name`。

用户说“我的模板 / 最近访问 / 已有 Base”时不要走模板中心：按上方 URL、`+title-resolve` 或已有 Base 候选列表路径处理。`drive +search --doc-types bitable` 只列用户可访问的云空间/Wiki Base 文件候选，不能查到模板中心公开模板。
